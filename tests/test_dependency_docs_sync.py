...
def _read_requirements_packages(req_path: Path) -> set[str]:
...
def _read_documented_packages_by_requirement_file(doc_text: str) -> dict[str, set[str]]:
    ...
    for match in bullet_pattern.finditer(doc_text):
        package_names = re.findall(r"`([^`]+)`", match.group("package_list"))
        documented[match.group("filename")] = {
            _normalize_package_name(name) for name in package_names
        }
...
def test_dependency_docs_match_requirements_files():
    ...
    documented_by_file = _read_documented_packages_by_requirement_file(doc_text)
    ...
    for filename in REQUIREMENT_FILES:
        expected = _read_requirements_packages(REPO_ROOT / filename)
