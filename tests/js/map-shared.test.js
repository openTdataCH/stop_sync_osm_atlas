...
describe('MapShared identities', () => {
  ...
  test('builds type-prefixed stable entity keys', () => {
    expect(window.MapShared.createEntityKey('atlas', 'ch:1:sloid:1'))
      .toBe('atlas:ch:1:sloid:1');
    expect(window.MapShared.createEntityKey('osm', 123))
      .toBe('osm:123');
    expect(window.MapShared.createEntityKey('gtfs', '8500:0:1'))
      .toBe('gtfs:8500:0:1');
  });

  test('returns null instead of creating an unstable key', () => {
    expect(window.MapShared.createEntityKey('gtfs', null)).toBeNull();
    expect(window.MapShared.createEntityKey('unknown', 3)).toBeNull();
  });
