...
from transport_matcher.core.models import SourceStop, Reference, MatchRecord, OsmNode
from transport_matcher.core.pipeline import MatchingContext, run_pipeline
from transport_matcher.core.predicates import BasePredicate
from transport_matcher.core.predicates.distance_matching import (
    NearestDistancePredicate,
    bipartite_match,
    bipartite_match_max_cardinality,
    GroupProximityPredicate,
)
from transport_matcher.core.predicates.exact_matching import ExactUicPredicate
from transport_matcher.core.predicates.name_matching import NameMatchPredicate
from transport_matcher.core.predicates.trio_distance_matching import TrioDistanceMatchingPredicate
from transport_matcher.core.predicates.route_matching_gtfs import RouteMatchPredicate
from transport_matcher.api import _predicates
from transport_matcher.profiles import SWITZERLAND
DEFAULT_PIPELINE = _predicates(SWITZERLAND)
from transport_matcher.core.problem_detection.context import ProblemContext
from transport_matcher.core.problem_detection.stop_predicates import contradicts_route_matching_problem
...
def _source_stop(source_id, lat, lon, station_ref, platform_code, name, operator):
    return SourceStop('atlas', source_id, lat, lon, name=name, platform_code=platform_code,
                      station_ref=station_ref, operator=operator,
                      references=(Reference('uic', station_ref, 'station'),) if station_ref else ())


def _source_row(source_id, number, platform_code, name, lat, lon):
    return _source_stop(source_id, lat, lon, number, platform_code, name, 'TEST')
...
def _build_ctx(
    ...
    *,
    ...
    source_state = SourceState(
        atlas_rows,
        duplicate_key_map={f'atlas:{key}': [f'atlas:{member}' for member in members] for key, members in (duplicate_sloid_map or {}).items()},
        route_evidence_by_key={f'atlas:{key}': value for key, value in (atlas_route_evidence_by_sloid or {}).items()},
    )
    nodes = [OsmNode(entry['node_id'], entry['lat'], entry['lon'], local_ref=entry.get('local_ref'), tags=entry['tags']) for entry in osm_entries]
    for entries in (node_routes or {}).values():
        for entry in entries:
            entry['route_id_normalized'] = normalize_route_id(entry.get('gtfs_route_id'))
    osm_state = OsmState.from_records(nodes, node_routes=node_routes, name_dirs=name_dirs)
    return MatchingContext(source=source_state, osm=osm_state, max_distance=50.0, profile=SWITZERLAND)
...
class TestBipartiteMatch:
    def test_equal_size_conflict_free(self):
        atlas = [
            _source_stop('a1', 47.0, 8.0, 'u1', '1', 'A', 'T'),
            _source_stop('a2', 47.0005, 8.0005, 'u1', '2', 'A', 'T'),
        ]
        osm = [
    ...
    def test_unequal_size_returns_empty(self):
        atlas = [_source_stop('a1', 47.0, 8.0, 'u1', '1', 'A', 'T')]
        osm = [
            OsmNode('o1', 47.0001, 8.0001, None, 'A', None, 'u1', '', '', 'stop_position', None, None, None, {}),
    ...
    def test_max_cardinality_allows_partial_matching(self):
        atlas = [
            _source_stop('a1', 47.0000, 8.0000, 'u1', '1', 'A', 'T'),
            _source_stop('a2', 47.0005, 8.0005, 'u1', '2', 'A', 'T'),
            _source_stop('a3', 47.0100, 8.0100, 'u1', '3', 'A', 'T'),
        ]
        osm = [
            OsmNode('o1', 47.0001, 8.0001, None, 'A', None, 'u1', '', '', 'stop_position', None, None, None, {}),
            OsmNode('o2', 47.0006, 8.0006, None, 'A', None, 'u1', '', '', 'stop_position', None, None, None, {}),
...
class TestPipelineRunner:
    def test_empty_predicates_returns_all_unmatched(self):
        ctx = _build_ctx(
            [_source_row('s1', 'u1', '1', 'Stop 1', 47.0, 8.0)],
        ...
        assert [node.source_id for node in output.unmatched_source] == ['s1']

    def test_multiple_predicates_chain(self):
        ctx = _build_ctx(
            [
                _source_row('s1', 'u1', '1', 'Stop 1', 47.0, 8.0),
                _source_row('s2', 'u2', '2', 'Stop 2', 47.1, 8.1),
            ],
            [
                _osm_entry('o1', 47.0, 8.0, uic_ref='u1', name='Stop 1'),
        ...
        assert len(output.unmatched_source) == 0


class TestCurrentPredicates:
    def test_exact_uic_predicate_single_candidate(self):
        ctx = _build_ctx(
            [_source_row('s1', '8503000', '1', 'Zürich HB', 47.3769, 8.5417)],
            [_osm_entry('osm_1', 47.3770, 8.5418, uic_ref='8503000', name='Zürich HB')],
        )

        ExactUicPredicate().run(ctx)

        assert [(record.source_node.source_id, record.osm_node.node_id, record.match_type) for record in ctx.all_matches] == [
            ('s1', 'osm_1', 'exact')
        ]
    ...
    def test_name_match_predicate_refines_by_local_ref(self):
        ctx = _build_ctx(
            [_source_row('s1', '8503000', '2', 'Zürich HB', 47.3769, 8.5417)],
            [
    ...
    def test_route_match_predicate_matches_by_gtfs_tokens(self):
        ...
        ctx.source._route_evidence_by_key = {
            'atlas:s1': {
                'gtfs': [
                    {
    ...
    def test_route_match_predicate_prefers_stronger_token_overlap(self):
        ctx = _build_ctx(
            [
                _source_row('s_weak', 'u1', '1', 'Stop', 47.00000, 8.00000),
                _source_row('s_strong', 'u1', '2', 'Stop', 47.00002, 8.00002),
            ],
        ...
        assert len(ctx.all_matches) == 1
        assert ctx.all_matches[0].source_node.source_id == 's_strong'
        assert ctx.all_matches[0].osm_node.node_id == 'osm_1'
        assert ctx.all_matches[0].match_type == 'route_gtfs_tokens'
    ...
    def test_route_match_predicate_skips_ambiguous_equal_token_matches(self):
        ctx = _build_ctx(
            [
                _source_row('s1', 'u1', '1', 'Stop', 47.00000, 8.00000),
                _source_row('s2', 'u1', '2', 'Stop', 47.00001, 8.00001),
            ],
    ...
    def test_route_matching_before_distance_avoids_direction_conflict(self):
        atlas_rows = [
            _source_row('s_bad', '', '', '', 46.5221688, 6.5661288),
            _source_row('s_good', '', '', '', 46.5222554, 6.5656973),
        ]
        osm_entries = [
        ...
        assert [(record.source_node.source_id, record.osm_node.node_id) for record in ctx.all_matches] == [
            ('s_good', 'osm_1')
        ]

    def test_contradicts_route_matching_problem_flags_direction_conflict(self):
        ctx = ProblemContext(
            source_route_evidence_by_key={
                'atlas:s_bad': {
                    'gtfs': [
                        {
                            'route_id': '91-m1-j26-1',
        ...
        record = MatchRecord(
            source_node=_source_stop('s_bad', 46.5221688, 6.5661288, '', '', '', 'TL'),
    ...
    def test_group_proximity_partial_matching_leaves_far_outlier_unmatched(self):
        ctx = _build_ctx(
            [
                _source_row('s1', 'u1', '1', 'Hub', 47.0000000, 8.0000000),
                _source_row('s2', 'u1', '2', 'Hub', 47.0005000, 8.0005000),
                _source_row('s3', 'u1', '3', 'Hub', 47.0050000, 8.0050000),
            ],
            [
        ...
        matched_sloids = {r.source_node.source_id for r in ctx.all_matches}
...
class TestStaleCandidateRegression:
    def test_nearest_distance_does_not_reuse_consumed_group_representative(self):
        ctx = _build_ctx(
            [
                _source_row('s1', 'u1', '', 'Stop', 47.0000000, 8.0000000),
                _source_row('s2', 'u1', '', 'Stop', 47.0002100, 8.0002100),
                _source_row('s3', 'u1', '', 'Stop', 47.0003250, 8.0003250),
            ],
            [
        ...
        ctx.osm._group_siblings = {
            'rep_a': ('osm_pair_tram', [ctx.osm._to_osm_node(ctx.osm._all_nodes['sib_a'])]),
            'rep_b': ('osm_pair_tram', [ctx.osm._to_osm_node(ctx.osm._all_nodes['sib_b'])]),
        }
        ...
        matched_pairs = [(record.source_node.source_id, record.osm_node.node_id, record.match_type) for record in ctx.all_matches]
        ...
        assert 's3' not in {record.source_node.source_id for record in ctx.all_matches}
...
class TestOsmGroupingPolicy:
    def test_uic_grouping_uses_perfect_count_branch_with_15m_reciprocal_pairs(self):
        ctx = _build_ctx(
            [
                _source_row('s1', 'u_pc', '1', 'Stop PC', 47.0, 8.0),
                _source_row('s2', 'u_pc', '2', 'Stop PC', 47.0001, 8.0001),
                _source_row('s3', 'u_pc', '3', 'Stop PC', 47.0002, 8.0002),
            ],
            [
        ...
        ctx.osm.build_groups(source_uic_counts={'u_pc': 3})
    ...
    def test_uic_perfect_count_uses_filtered_atlas_count_only(self):
        ctx = _build_ctx(
            [
                _source_row('s_near_1', 'u_filtered', '1', 'Filtered Stop', 47.0000000, 8.0000000),
                _source_row('s_near_2', 'u_filtered', '2', 'Filtered Stop', 47.0001000, 8.0001000),
                _source_row('s_far_1', 'u_filtered', '3', 'Filtered Stop', 47.0100000, 8.0100000),
                _source_row('s_far_2', 'u_filtered', '4', 'Filtered Stop', 47.0110000, 8.0110000),
            ],
            [
        ...
        ctx.osm.build_groups(
            source_uic_counts={'u_filtered': 4},
            source_uic_nearest_osm_distances={'u_filtered': [2.0, 3.0, 250.0, 270.0]},
        )
    ...
    def test_name_grouping_uses_perfect_count_branch_with_15m_reciprocal_pairs(self):
        ctx = _build_ctx(
            [
                _source_row('s1', 'u_name_pc', '1', 'Name PC', 47.0, 8.0),
                _source_row('s2', 'u_name_pc', '2', 'Name PC', 47.0001, 8.0001),
            ],
            [
        ...
        ctx.osm.build_groups(
            source_uic_counts={'u_name_pc': 2},
            source_name_to_station_ref={'Name PC': 'u_name_pc'},
        )
    ...
    def test_tram_grouping_uses_perfect_count_branch_with_15m_reciprocal_pairs(self):
        ctx = _build_ctx(
            [
                _source_row('s1', 'u_tram_pc', '1', 'Tram PC', 47.0, 8.0),
                _source_row('s2', 'u_tram_pc', '2', 'Tram PC', 47.0001, 8.0001),
            ],
            [
        ...
        ctx.osm.build_groups(source_uic_counts={'u_tram_pc': 2})
    ...
    def test_uic_grouping_accepts_incomplete_relaxed_pairs(self):
        ctx = _build_ctx(
            [_source_row('s1', 'u1', '1', 'Stop', 47.0, 8.0)],
            [
        ...
        ctx.osm.build_groups(source_uic_counts={'u1': 3})
    ...
    def test_name_grouping_accepts_incomplete_relaxed_pairs(self):
        ctx = _build_ctx(
            [_source_row('s1', 'u_name', '1', 'Name Stop', 47.0, 8.0)],
            [
        ...
        ctx.osm.build_groups(
            source_uic_counts={'u_name': 3},
            source_name_to_station_ref={'Name Stop': 'u_name'},
        )
    ...
    def test_tram_grouping_accepts_incomplete_relaxed_pairs_without_outlier_logic(self):
        ctx = _build_ctx(
            [_source_row('s1', 'u_tram', '1', 'Tram Stop', 47.0, 8.0)],
            [
        ...
        ctx.osm.build_groups(
            source_uic_counts={'u_tram': 3},
            source_uic_nearest_osm_distances={'u_tram': [35.0, 10.0, 9.0]},
        )
    ...
    def test_grouped_lookup_inherits_uic_ref_from_sibling(self):
        ctx = _build_ctx(
            [_source_row('s1', 'u_anchor', '1', 'Anchor Name', 47.0, 8.0)],
            [
        ...
        ctx.osm.build_groups(
            source_uic_counts={'u_anchor': 1},
            source_name_to_station_ref={'Anchor Name': 'u_anchor'},
        )
...
class TestNearestDistanceMultiPass:
    def test_second_single_pass_can_match_after_ratio_consumes_competitor(self):
        ctx = _build_ctx(
            [
                _source_row('s_target', 'u_aerial', '', 'Aerial', 46.11343836621, 7.30420757178),
                _source_row('s_competitor', 'u_aerial', '', 'Aerial', 46.1136200, 7.3044000),
            ],
            [
                _osm_entry(
                    'way_208449383',
        ...
        by_sloid = {record.source_node.source_id: record for record in ctx.all_matches}
...
class TestOsmTrioMatching:
    def test_trio_commit_does_not_propagate_middle_node(self):
        ctx = _build_ctx(
            [
                _source_row('a1', 'u_trio', '1', 'Trio Stop', 47.3000000, 8.3000000),
                _source_row('a2', 'u_trio', '2', 'Trio Stop', 47.3002000, 8.3002000),
            ],
            [
        ...
        ctx.osm.build_groups(source_uic_counts={'u_trio': 2})

        rep_entity = next(e for e in ctx.osm.get_by_uic('u_trio') if e.node_id == 'side_other' or e.node_id == 'side_rep')
        atlas_entry = next(e for e in ctx.source.get_unmatched_records() if e.source_id == 'a1')
    ...
    def test_trio_stage_matches_two_side_nodes_and_leaves_middle_unmatched(self):
        ctx = _build_ctx(
            [
                _source_row('a1', 'u_trio', '1', 'Trio Stop', 47.4000000, 8.4000000),
                _source_row('a2', 'u_trio', '2', 'Trio Stop', 47.4003000, 8.4003000),
            ],
            [
    ...
    def test_non_local_trio_is_not_registered_and_can_fall_back_to_pair_grouping(self):
        ctx = _build_ctx(
            [
                _source_row('a1', 'u_trio', '1', 'Trio Stop', 47.5000000, 8.5000000),
                _source_row('a2', 'u_trio', '2', 'Trio Stop', 47.5003000, 8.5003000),
            ],
            [
