...
def query_overpass(session: Optional[requests.Session] = None, *, output_path="data/raw/osm_data.xml", country_code="CH", overpass_url=None):
    """
    ...
    query = """
        ...
        """
    query = query.replace('"CH"', f'"{country_code.upper()}"')
    ...
    if output_path is not None:
        ...
        print(f"Raw OSM data saved to {output_path}")
...
def process_osm_routes_data(xml_data, out_dir="data/processed/"):
    ...
    def _write_csv(path, rows, fieldnames):
        import pandas as pd
        products[os.path.basename(path).removesuffix('.csv')] = pd.DataFrame(rows, columns=fieldnames)
        if out_dir is None:
            return
    ...
    _write_csv(
        os.path.join(out_dir or '', 'osm_route_masters.csv'),
        route_master_rows,
        [
    ...
    _write_csv(
        os.path.join(out_dir or '', 'osm_route_master_tags.csv'),
        route_master_tag_rows,
        ['run_id', 'route_master_id', 'tag_key', 'tag_value'],
    )
    _write_csv(
        os.path.join(out_dir or '', 'osm_route_master_members.csv'),
        route_master_member_rows,
        ['run_id', 'route_master_id', 'relation_id', 'member_sequence', 'member_role'],
    )
    _write_csv(
        os.path.join(out_dir or '', 'osm_route_relations.csv'),
        routes_rows,
        [
    ...
    _write_csv(
        os.path.join(out_dir or '', 'osm_route_relation_tags.csv'),
        tags_rows,
        ['run_id', 'relation_id', 'tag_key', 'tag_value'],
    )
    _write_csv(
        os.path.join(out_dir or '', 'osm_route_relation_members.csv'),
        members_rows,
        ['run_id', 'relation_id', 'member_type', 'member_ref', 'member_role', 'member_sequence', 'resolved_node_id', 'direction_id_derived'],
    )
    _write_csv(
        os.path.join(out_dir or '', 'osm_route_relation_stops.csv'),
        relation_stop_rows,
