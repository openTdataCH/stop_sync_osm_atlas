...
def download_and_extract_gtfs(gtfs_url, gtfs_folder="data/raw/gtfs"):
...
def _load_gtfs_metadata(gtfs_folder, stop_selector):
    """Shared stop/trip parsing semantics for native and comparison readers."""
    ...
    swiss_stops = stop_selector(all_stops) if stop_selector is not None else _select_swiss_gtfs_stops(all_stops)
    ...
    return all_stops, swiss_stops, trips_all


def load_gtfs_data_streaming(gtfs_folder: str, *, stop_selector=None, stage_dir=None):
    ...
    all_stops, swiss_stops, trips_all = _load_gtfs_metadata(gtfs_folder, stop_selector)
    swiss_stop_ids = set(swiss_stops['stop_id'].astype(str))
    ...
    trip_stop_times_path = os.path.join(stage_dir or gtfs_folder, GTFS_TRIP_STOP_TIMES_STAGE_FILENAME)
    try:
    ...
    if relevant_route_ids:
        all_routes = pd.read_csv(
            routes_path,
            usecols=lambda column: column in {'route_id', 'agency_id', 'route_short_name', 'route_long_name', 'route_desc', 'route_type'},
        ...
        if 'route_id' not in all_routes:
            raise ValueError('GTFS routes.txt requires route_id')
        for column in ('agency_id', 'route_short_name', 'route_long_name', 'route_desc', 'route_type'):
            if column not in all_routes:
                all_routes[column] = None
...
def build_integrated_gtfs_data_streaming(
    ...
    if stats_path is not None:
        ...
        with open(stats_path, 'w', encoding='utf-8') as handle:
            json.dump(gtfs_payload['mapping_stats_export'], handle, indent=2)
...
def build_gtfs_db_payload_rows(
    ...
    # Direct original_stop_id matches may cross UIC numbers. Resolve coordinates
    # from the full SLOID lookup so valid identities never lose their position.
