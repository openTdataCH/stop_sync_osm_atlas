...
def _build_base_lookups(base_data: MatchingOutput | None, known_sloids: set[str]) -> dict[str, Any]:
    ...
    def register_atlas_node(node: Any) -> None:
        sloid = _to_text(getattr(node, 'key', None))
        if sloid is None:
            return
        atlas_stop_lookup[sloid] = {
            ...
            'uic_ref': _to_text(getattr(node, 'station_ref', None)),
            'designation': _to_text(getattr(node, 'platform_code', None)),
            'designation_official': _to_text(getattr(node, 'name', None)),
        }
    ...
    for match in getattr(base_data, 'matched', []):
        register_atlas_node(match.source_node)
        register_osm_node(match.osm_node)
        atlas_sloid = _to_text(match.source_node.key)
    ...
    for atlas_node in getattr(base_data, 'unmatched_source', []):
...
def _build_atlas_stop_call_row(
    ...
    resolved_variants = [
        key for key in _parse_json_list(row.get('resolved_sloid_variants') or row.get('sloid_variants'))
    ...
    return {
        ...
        'resolved_sloid_variants': json.dumps(resolved_variants) if resolved_variants else None,
...
def _build_atlas_source_rows(
    ...
    for row in _records(atlas_line_source):
        ...
        line_family_rows.append({
            ...
            'route_id_normalized': _first_non_empty(row.get('route_id_normalized'), atlas_line_id),
...
def _resolve_osm_stop_fields(node_id: str | None, row: dict[str, Any], lookups: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        'canonical_stop_key': canonical_stop_key,
        'resolved_source_key': canonical_stop_key if canonical_stop_key in atlas_stop_lookup else None,
        'stop_label': _first_non_empty(
            row.get('stop_label'),
...
def _build_osm_source_rows(all_route_data: dict[str, pd.DataFrame], lookups: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in _records(all_route_data.get('osm_route_masters')):
        ...
        route_master_rows.append({
            ...
            'route_id_normalized': _to_text(row.get('route_id_normalized')),
    ...
    if explicit_relation_stops is not None and not explicit_relation_stops.empty:
        relation_member_source = None
    elif relation_member_source is not None and 'resolved_node_id' in relation_member_source:
        resolved = relation_member_source['resolved_node_id']
        relation_member_source = relation_member_source.loc[resolved.notna() & resolved.ne('')]
...
def _build_line_family_rows(
    ...
    for row in atlas_rows['atlas_line_families']:
        ...
        line_family_rows.append({
            ...
            'normalized_route_id': _first_non_empty(row.get('route_id_normalized'), row['atlas_line_id']),
    ...
    for relation in osm_rows['osm_route_relations']:
        ...
        normalized_gtfs_route_id = _first_non_empty(relation.get('route_id_normalized'), gtfs_route_id)
    ...
    for group in family_groups.values():
        ...
        normalized_route_id = _first_non_empty(route_master_row.get('route_id_normalized'), representative_relation.get('route_id_normalized'), gtfs_route_id, representative_relation.get('ref'))
        line_family_rows.append({
            'id': family_id,
            'source': 'osm',
    ...
    for row in osm_rows['osm_route_relations']:
        ...
        for call_row in call_rows:
            stop_call = {
                ...
                'stop_sequence': call_row['stop_sequence'],
                'source_stop_id': None,
                'source_sloid': call_row.get('resolved_source_key'),
...
def _candidate_line_family_pairs(atlas_families, osm_families):
    indexes = {key: defaultdict(list) for key in ('gtfs_route_id', 'normalized_route_id', 'display_route_id')}
    for osm in osm_families:
        if not osm.get('is_non_gtfs'):
            for key, index in indexes.items():
                value = _to_text(osm.get(key))
                if value:
                    index[value].append(osm)
    candidates = []
    for atlas in atlas_families:
        possible = {}
        for key, value in (('gtfs_route_id', atlas.get('gtfs_route_id')),
                           ('normalized_route_id', atlas.get('normalized_route_id')),
                           ('display_route_id', atlas.get('gtfs_route_id'))):
            for osm in indexes[key].get(_to_text(value), ()):
                possible[osm['id']] = osm
        for osm in possible.values():
            score, reason = _score_line_family_pair(atlas, osm)
            if score > 0 and reason:
                candidates.append((score, atlas, osm, reason))
    return candidates
...
def _resolved_stop_identities(stop_call: dict[str, Any]) -> set[str]:
    ...
    # Canonical fallback keys can be opaque, unresolved source IDs. Only
    # explicit resolved references establish physical source identity.
    resolved_key = _to_text(stop_call.get('resolved_source_key'))
    if resolved_key:
        identities.add(resolved_key)

    return identities


def _stop_identity(call):
    return frozenset(_resolved_stop_identities(call)), _to_text(call.get('uic_ref'))


def _match_stop_identities(atlas, osm):
...
def _match_stop_calls(atlas_call, osm_call):
    return _match_stop_identities(_stop_identity(atlas_call), _stop_identity(osm_call))


def _count_stop_sequence_matches(atlas_calls, osm_calls):
    # Same LCS score as the detailed alignment, using O(m) memory. Publication
    # needs a score, not a traceback containing copies of every call dictionary.
    atlas = [_stop_identity(call) for call in atlas_calls]
    osm = [_stop_identity(call) for call in osm_calls]
    previous = [0] * (len(osm) + 1)
    for left_call in atlas:
        current = [0]
        for j, right_call in enumerate(osm):
            current.append(previous[j] + 1 if _match_stop_identities(left_call, right_call)
...
def _align_stop_sequences(atlas_calls: list[dict[str, Any]], osm_calls: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int]:
    ...
    cols = len(osm_calls)
    atlas_identities = [_stop_identity(call) for call in atlas_calls]
    osm_identities = [_stop_identity(call) for call in osm_calls]
    ...
    for i in range(1, rows + 1):
        for j in range(1, cols + 1):
            match_type = _match_stop_identities(atlas_identities[i - 1], osm_identities[j - 1])
...
def _score_itinerary_pair(
    ...
    *,
    ...
    min_stop_ratio: float = _ITINERARY_MATCH_MIN_RATIO,
    ...
    if include_alignments:
        ...
        alignments = []
        matched_stop_count = _count_stop_sequence_matches(atlas_calls, osm_calls)
    stop_score = round(matched_stop_count / max(len(atlas_calls), len(osm_calls), 1), 4)
    directions_known = _to_text(atlas_itinerary.get('direction_id')) is not None and _to_text(osm_itinerary.get('direction_id')) is not None
    direction_eligible = direction_matches or (not require_direction_match and not directions_known)
    is_eligible = direction_eligible and stop_score >= min_stop_ratio
    if not direction_eligible:
        match_reason = 'direction_mismatch'
    elif is_eligible:
        match_reason = 'ordered_stop_match' if direction_matches else 'ordered_stop_match_direction_unknown'
    else:
...
def build_route_write_payload(
    ...
    *,
    ...
    """Compare route families and ordered itineraries, returning complete results."""
    if not 0 < min_stop_ratio <= 1:
        raise ValueError('min_stop_ratio must be greater than zero and at most one')
    ...
    candidate_pairs = _candidate_line_family_pairs(atlas_line_families, osm_line_families)
