# E1.2 Swiss Integration: GTFS and ATLAS
...
The Swiss integration exports the following run-specific metrics in bundle metadata. Values are deliberately not embedded in this canonical engine documentation; consumers and deployment dashboards render the snapshot they imported.

- `gtfs_atlas.atlas.total`
- `gtfs_atlas.atlas.touched_by_gtfs_routes`
- `gtfs_atlas.atlas.coverage_percent`
...
GTFS `stop_id` mapping metrics are `gtfs_atlas.gtfs_stop_ids.total`, `matched_to_atlas`, `unmatched` and `coverage_percent`. Assignment counters are exported under `gtfs_atlas.assignments` for `original_stop_id`, `strict`, `coordinate_proximity` and `unique_number_fallback`.

The generic GTFS adapter uses source-neutral route product names (`source_line_families`, `source_itineraries`, `source_itinerary_stop_calls`) and source-neutral identity fields. The `atlas_*` CSV names below are specific to the Swiss cache and the legacy schema-1 result projection.
...
See [Pipeline Performance](5.2%20Pipeline%20Performance.md) for algorithms, cache dependencies, memory limits, logs and reproducible benchmarks.
...
The Routes tab diagnostic map reads the imported versions of these payloads from the `gtfs_stops_raw` and `gtfs_stop_identity_resolution` tables. See [A2.6 Routes Pages](../../documentation/2.6%20Routes%20Pages.md).
...
This page describes the Swiss adapter. The generic `adapters.gtfs.read_gtfs()` path does not require Swiss ID conventions, geography filters or SLOID integration; see [E1 Data Acquisition & Processing](1.%20Download%20and%20process%20data.md).
