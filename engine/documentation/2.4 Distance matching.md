# E2.4 Distance Matching

Distance matching uses spatial proximity to produce matches. It consists of **four predicate classes** contributing **seven sequential predicate executions**. `GroupProximityPredicate` is instantiated twice with different distance and output settings, while `NearestDistancePredicate` is instantiated three times with different modes.
...
The numeric values below are the current generic and Swiss profile defaults. Custom profiles can override normal/long distance, ratio, isolation, and grouping thresholds.
...
| Predicate Class | `match_type` | Exported counter suffix |
|-----------------|--------------|-------------------------|
| `TrioDistanceMatchingPredicate` | `distance_matching_trio` | `stage0_trio` |
| `GroupProximityPredicate` | `distance_matching_1_*` | `stage1_group` |
| `GroupProximityPredicate` (long-distance configuration) | `long_distance_group_proximity_*` | `stage1b_long_group` |
| `LocalRefDistancePredicate` | `distance_matching_2` | `stage2_local_ref` |
| `NearestDistancePredicate` (single pass 1) | `distance_matching_3a` | `stage3a_single_pass1` |
| `NearestDistancePredicate` (ratio) | `distance_matching_3b` | `stage3b_relative` |
| `NearestDistancePredicate` (single pass 2) | `distance_matching_3a_second_pass` | `stage3a_single_pass2` |

Run-specific values are exported with the result rather than embedded in this page.

## Predicates

Distance matching contributes seven sequential predicate executions in the pipeline: trio, group proximity at 50m, group proximity at 150m, local-ref, nearest 3a, nearest 3b, and nearest 3a retry. These runs do **not** execute in parallel. Each run operates on `SourceStop` and `OsmNode` domain entities and records matches via `ctx.commit()`. The runner refreshes unmatched records between calls, so every stage sees the updated post-commit state.
...
1. **UIC reference**: `SourceStop.station_ref` ↔ `OsmNode.uic_ref`
2. **UIC name**: `SourceStop.name` ↔ `OsmNode.uic_name`
3. **Official name**: `SourceStop.name` ↔ `OsmNode.name`
...
## `GroupProximityPredicate` Long-Distance Configuration - Same Group Logic at 150m
...
Matches where `SourceStop.platform_code` exactly equals `OsmNode.local_ref`, case-insensitively, within 50m. Uses KDTree spatial index via `batch_query_radius()` to find nearby candidates. When multiple candidates match, the closest wins.
...
**local_ref contradiction filter**: Before evaluating candidates, any `OsmNode` whose `local_ref` contradicts the `SourceStop.platform_code` is excluded (case-insensitive comparison). If either value is empty, no contradiction exists and the candidate passes through. This prevents false matches where a nearby OSM node clearly belongs to a different stop.
...
**Case 3a — Single candidate**: If exactly **one** non-contradicting, unused `OsmNode` that passes `OsmNode.is_station` filtering exists within 50m of an `SourceStop` → match (`distance_matching_3a` on pass 1, `distance_matching_3a_second_pass` on pass 3).
...
Profile settings: `ratio_min_second_distance = 10` metres and `ratio_factor = 4`.
...
The `LocalRefDistancePredicate` and `NearestDistancePredicate` use **batched queries**: all unmatched `SourceStop` coordinates are collected into a list and passed to `OsmState.batch_query_radius()`, which calls `query_ball_point(points, workers=-1)` once — delegating the entire loop to SciPy's multithreaded C backend.
...
The spatial index is built once from available nodes that pass the current `include_stations` / `OsmNode.is_station` filter and reused across queries. `used_ids` and sibling nodes are filtered at query time, not at build time. Implementation is in [utils/spatial_index.py](../src/transport_matcher/core/utils/spatial_index.py) and [state.py](../src/transport_matcher/core/state.py).

## No-Nearby-OSM Detection

After all predicates complete, the engine flags unmatched source entries that have no OSM node within `profile.isolation_radius` (50 m by default). This does not create matches — it identifies entries for problem detection.

## Used-Node Prevention

**No single OSM representative** is matched more than once within the distance predicates — `ctx.commit()` immediately locks nodes via `mark_used()`, and later rows re-check that live state before committing. However, this does not imply a strict 1:1 relationship at the station level: since OSM often includes multiple nodes for a single stop (e.g. `platform` and `stop_position`), the distance matching process can validly match these distinct `OsmNode` entities to separate `SourceStop` entries.
...
Distance predicates are in [predicates/distance_matching.py](../src/transport_matcher/core/predicates/distance_matching.py) plus [predicates/trio_distance_matching.py](../src/transport_matcher/core/predicates/trio_distance_matching.py), with spatial utilities in [utils/spatial_index.py](../src/transport_matcher/core/utils/spatial_index.py) and [state.py](../src/transport_matcher/core/state.py).
