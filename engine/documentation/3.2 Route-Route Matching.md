# E3.2 Route-Route Matching

This page explains how route equivalency is built inside the engine, after stop matching in `src/transport_matcher/routes.py`.

Important distinction:

- [E2.3 Stop-stop matching using routes](2.3%20Stop-stop%20matching%20using%20routes.md) is stop-level matching (ATLAS stop <-> OSM stop)
- this page is route-level linking (ATLAS line family <-> OSM family, then itinerary <-> itinerary)

## Where It Happens In The Pipeline

Route-route matching runs after stop matching has already produced the set of normalized source keys.

```mermaid
flowchart LR
   A[Receive adapter route frames] --> B[Resolve normalized source identities]
   ...
   E --> F[Export line_family_matches and itinerary_matches]
```

A per-run `RouteState` establishes initial route evidence for stop matching. After stop matching, `build_route_write_payload()` compares complete route families and ordered itineraries using the resulting stop identities. Both computations belong to the engine.

## Inputs

The route comparator receives normalized DataFrames from adapters. The names below are logical product keys; the Swiss ATLAS products may be loaded from same-named cache CSVs, while OSM products normally exist only in memory for the current XML:
...
The base stop-matching output matters because it lets the route comparator reuse already matched physical stop identities when comparing OSM and ATLAS stop sequences.

## Step 1: Normalize Line Families

The comparator first creates comparable family rows in `line_families`.
...
After normalization, the comparator scores candidate ATLAS/OSM family pairs.
...
Inside each side, the comparator turns source itineraries into shared `itineraries` and `stop_calls` rows.
...
For each matched family, the comparator scores every possible ATLAS/OSM itinerary pair.

The rules are:

1. In the Swiss profile, both `direction_id` values must be present and match exactly. Generic profiles allow unknown direction when ordered stop evidence is sufficient; known conflicting directions still reject the pair.
2. stop sequences are aligned in order
3. stop-call equality uses explicit resolved source identities first; prefixes or arbitrary canonical strings are not treated as proof of resolution
...
The resulting `itinerary_matches` rows keep the debugging scores written by the comparator: `direction_score`, `stop_score`, `geometry_score` (currently `NULL`), `overall_score`, and `match_reason`.
...
`MatchingProfile.itinerary_require_direction` controls missing-direction eligibility, and `itinerary_min_stop_ratio` controls the default 0.8 threshold. An eligible generic pair with missing direction records `ordered_stop_match_direction_unknown` and `direction_score=0`, preserving the absence of direction evidence.
