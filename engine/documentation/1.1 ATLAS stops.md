...
> [!NOTE]
> The standalone acquisition adapter caches ATLAS filtering, the downloaded GTFS feed, parsed GTFS products, and GTFS-to-ATLAS mapping as separate stages. HTTP validators establish remote freshness and SHA-256 fingerprints verify local products. For example, an ATLAS-only change can reuse the parsed GTFS feed and rebuild only the dependent identity/itinerary mapping. OSM is refreshed for every full acquisition run. See [7.4 Source Snapshot Caching](7.4%20Atlas-Cached%20Import%20Optimization.md) and [7.5 Pipeline Performance](7.5%20Pipeline%20Performance.md).
>
...
2. **Routes tab diagnostic map** -> [6.6 Routes Pages](../../documentation/6.6%20Routes%20Pages.md)
