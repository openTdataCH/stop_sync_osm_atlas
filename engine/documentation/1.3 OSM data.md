# E1.3 OSM Input and Acquisition

OpenStreetMap provides the stop geometry and route-relation side of the comparison pipeline.

The OSM side is deliberately split into three responsibilities:

1. `acquisition/overpass.py` optionally fetches public-transport elements and relations
2. `adapters/osm.py` parses positioned elements into normalized `OsmNode` records and `OsmState`
3. `adapters/osm_routes.py` derives route-master and route-relation frames preserving OSM PTv2 structure

Both adapters consume the same explicit XML snapshot and perform no network access. `adapters/get_osm_data.py` is a compatibility shim for the former combined module.
...
`adapters.osm.read_osm()` parses the raw XML into `OsmNode` entities. The runtime keeps the following stop-level attributes because the matching predicates and route export depend on them.
...
## Route Frames and Optional CSV Exports

The adapter produces entity-first route frames that preserve the OSM PTv2 hierarchy. The runner derives them in memory once from the current XML. The CSV names below describe the optional diagnostic exports when an explicit `out_dir` is supplied; normal acquisition now persists the XML without redundantly exporting/reloading OSM route CSVs.
...
| `synthetic_family_key` | Fallback grouping key used by engine route comparison |
...
The `osm_route_relation_stops` frame is the OSM-side stop-call source for engine route comparison.
...
| Tag | Purpose | Used In |
|-----|---------|---------|
| `uic_ref` | Exact stop identity and route-stop fallback | Exact matching, route comparator |
...
Consumers can derive the number of OSM elements carrying route evidence from the exported `route_evidence` extension. A current deployment value is not part of this canonical input-format documentation.

## Explicit parsing boundary

`read_osm(xml_path)` receives the file path explicitly and supplies records/evidence to `OsmState.from_records()`. OSM identities include element type; nodes and ways sharing an ID or position remain distinct. Supported way geometry still uses the Overpass center or available member-node mean. Full relation geometry matching is not implemented.

`process_osm_routes_data(xml_text, out_dir=None)` returns route products in memory. An explicit output directory enables diagnostic CSV exports; normal acquisition leaves route construction to the runner. Empty products write headers and remove stale rows; the standalone Swiss runner always derives route products from its current XML. The downloader returns source data without updating app metadata or status.

Stop relation members without supported geometry are counted in the `unsupported_or_external_osm_route_members` diagnostic, with sample typed IDs. Their raw membership remains reproducible from the fingerprinted XML/source products, but they are excluded from normalized itinerary calls under the preserved Swiss extraction policy.
