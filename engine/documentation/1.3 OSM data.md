...
The OSM adapter in `src/transport_matcher/adapters/get_osm_data.py` performs two distinct tasks:
...
The same raw XML is also parsed by `src/transport_matcher/adapters/osm.py` into normalized `OsmNode` records and `OsmState`, which keeps the stop attributes needed by the matching runtime.
