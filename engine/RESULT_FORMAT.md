...
Schema 1 route section names are `atlas_line_families`, `osm_route_relations`, `line_families`, `itineraries`, `stop_calls`, `line_family_matches`, `itinerary_matches`, and summary counters such as `matched_routes` and `skipped_sloids`. The `atlas_*` and `*_sloid` names are deprecated compatibility names in the version-1 wire format; they represent the configured source side even for a generic GTFS feed. Generic adapters use `source_*` names internally and the schema-1 serializer performs the legacy projection. Source reference values are always engine source keys. A future schema version can remove these compatibility names only with coordinated consumer support.
