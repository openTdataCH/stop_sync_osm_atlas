...
flowchart TB
    subgraph Registration
        R1[Submit Email/Password] --> R2{Turnstile Valid?}
        R2 -->|Yes| R3[Hash Password<br/>Argon2]
        R3 --> R4[Create Account]
        R4 --> R5[Send Verification<br/>Email]
    end
    ...
    subgraph Security
        S1[Session Cookie<br/>HttpOnly + SameSite]
        S2[CSRF Token<br/>All Forms]
        S3[Rate Limiting<br/>10/min Login]
        S4[Talisman Headers<br/>CSP + HSTS]
    end
```
...
| Email Service | `backend/services/email.py` | Verification emails via AWS SES |
...
### Registration
1. User submits email + password → Turnstile validates
2. Email normalized, checked for duplicates
3. Account created with Argon2-hashed password
4. Verification email sent (if SES configured)

### Login
1. Credentials submitted → Turnstile validated → Lockout checked
...
| CAPTCHA | Cloudflare Turnstile | Optional on register/login |
...
| `AUTH_DATABASE_URI` | Yes | Auth database connection |
| `TOTP_SECRET_ENC_KEY` | For 2FA | Fernet key for TOTP encryption |
| `FORCE_HTTPS` | Production | Enable HTTPS enforcement |
| `TURNSTILE_SITE_KEY` | Optional | Cloudflare CAPTCHA |
| `AWS_REGION` | For email | SES region |
| `SES_FROM_EMAIL` | For email | Sender address |
