...
- [A2.3.1 Global Stats precomputed approach](2.3.1%20Global%20Stats%20precomputed%20approach.md)
...
|---|---|---|---|
| Configured source identifier | `source:agency/stop:123` | `atlas` | `sloid` |
| UIC station code (seven digits starting with `85`, optionally followed by `:<digits>`) | `8503000` | `station` | `station` |
...
`source:` accepts arbitrary source identifiers, including IDs containing spaces or numeric IDs that would otherwise be interpreted as OSM/UIC identifiers. The compatibility token names `atlas`/`sloid` remain internal; displayed source and ID labels come from `ReviewConfig`. Swiss shortcuts stay available, while their hints are only shown for the GTFS identity capability. Route search is rejected when `capabilities.routes` is false, and route hints/actions are hidden.
