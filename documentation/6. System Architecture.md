...
- `redis`: Optional shared backend for multi-worker rate limiting and Redis-backed pipeline coordination.
- `migrator`: Fast, one-shot migration service (`flask db upgrade`). Starts, applies migrations, and dies.

Also defined for test workflows:
...
## Operational Notes
- Production vs Development is separated via Compose.
...
- [6.4 Frontend Asset Management](6.4%20Frontend%20Asset%20Management.md): Vendored frontend libraries, Font Awesome optimization, and sync pipeline
