...
```mermaid
flowchart TB
    IN["Unmatched ATLAS"] --> Stage1

    subgraph Stage1["group_proximity"]
    ...
    Stage3 --> UA["Unmatched → Route stage"]
```
...
The `local_ref_distance` and `nearest_distance` predicates use **batched queries**: all unmatched ATLAS coordinates are collected into a single numpy array and passed to `OsmIndex.batch_query_radius()`, which calls `query_ball_point(points, workers=-1)` once — delegating the entire loop to SciPy's multithreaded C backend.
...
The spatial index is built once from available (unused, non-station) nodes and reused across queries. Implementation is in [spatial_index.py](../matching_process/spatial_index.py) and [state.py](../matching_process/state.py).
...
All functions are in [distance_matching.py](../matching_process/distance_matching.py), with spatial utilities in [spatial_index.py](../matching_process/spatial_index.py) and [state.py](../matching_process/state.py).
