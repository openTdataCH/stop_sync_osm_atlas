...
Distance matching is the **third stage** of the pipeline, producing matches by leveraging spatial proximity.
...
flowchart TB
    ...
    Stage3 --> U2(("Update Used<br/>OSM node IDs"))
```
...
|-------|--------|---------|
| 1 | Group proximity | {{stat:matching_stages.distance.breakdown.stage1_group}} |
| 2 | Exact `local_ref` | {{stat:matching_stages.distance.breakdown.stage2_local_ref}} |
| 3a | Single candidate within 50m | {{stat:matching_stages.distance.breakdown.stage3a_single}} |
| 3b | Relative distance (ratio test) | {{stat:matching_stages.distance.breakdown.stage3b_relative}} |
...
## Flagging Isolated ATLAS Entries

**Function**: `no_nearby_counterpart()`

This operation does **not create matches**. Instead, it **flags** ATLAS entries that have no OSM nodes within the 50m isolation radius. These entries are marked for later problem detection.

The isolation radius is taken from `matching_process.problem_detection.get_isolation_radius()` (currently **50m**).

- These entries are forwarded to the Route Matching stage for potential matching via transit route data
...
This ensures that **no single OSM node** is matched more than once within this stage. However, this does not imply a strict 1:1 relationship at the station level: since OSM often includes multiple nodes for a single stop (e.g. `platform` and `stop_position`), the distance matching process can validly match these distinct OSM nodes to separate ATLAS entries (resulting in a one-to-many relationship from the station perspective).

Note: As mentioned the overall pipeline is not strictly 1:1. Some other stages intentionally create many-to-one matches (e.g. exact UIC collapse or duplicate propagation).
