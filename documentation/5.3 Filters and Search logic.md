...
- [5.3.1 Global Stats Query Cost and Optimization](5.3.1%20Global%20Stats.md)
...
- `stop_position`
- `aerialway_station`
- `non_node_osm_stop` (OSM entries derived from ways, `is_way=true`)

### 4.2 OSM Group Predicate (Pairs/Trios)
...
## 11. Global Stats Endpoint Semantics and Routing

`/api/global_stats` follows the same predicate algebra as the rest of this document.

Implementation is now routed by request type:

- broad filter requests use precomputed `global_stats_filter_rows`
- identifier-search requests (`station_filter`) use canonical source-table SQL
- top-N requests (`top_n`) use canonical source-table SQL
...
This preserves trio-middle effective-match behavior across map rendering and global summary stats.
...
This avoids drift between counts and map behavior.

### 11.3 Global vs Viewport Scope

`/api/global_stats` is filter-scoped, not viewport-scoped:

- no `min_lat`, `max_lat`, `min_lon`, `max_lon`
- summarizes full filtered dataset
- `/api/data` remains viewport-scoped

### 11.4 Exactness Guarantee

Both routed paths compute exact `DISTINCT` metrics and return the same payload schema.

Materialized rows accelerate broad filter cases by avoiding request-time joins, while selective search/top-N requests keep canonical semantics.