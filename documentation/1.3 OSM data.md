...
The OSM adapter in `engine/src/transport_matcher/adapters/get_osm_data.py` performs two distinct tasks:

1. fetch Swiss public-transport stop elements from Overpass
2. derive route-master and route-relation frames that preserve OSM PTv2 structure for engine comparison

The same raw XML is also parsed by `engine/src/transport_matcher/adapters/osm.py` into normalized `OsmNode` records and `OsmState`, which keeps the stop attributes needed by the matching runtime.
