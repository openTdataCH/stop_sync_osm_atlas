...
OpenStreetMap provides community-maintained public transport data. We fetch Swiss PT nodes together with both `type=route` and `type=route_master` relations via Overpass API.
...
In addition to OSM nodes, we ingest a narrow subset of OSM ways as virtual stop elements.
...
|------|-------------|---------|
| `osm_route_masters.csv` | One row per OSM `type=route_master` relation with line-level metadata. | Route import |
| `osm_route_master_tags.csv` | Exploded key/value tags for each route-master relation. | Route import |
| `osm_route_master_members.csv` | Ordered members of each route_master relation. | Route import |
| `osm_route_relations.csv` | One row per OSM `type=route` relation with core itinerary metadata (`route`, `name`, `ref`, `operator`, `network`, `gtfs_route_id`, ...). | Route import, `RouteState` |
| `osm_route_relation_tags.csv` | Exploded key/value tags for each route relation. | Route import |
| `osm_route_relation_members.csv` | Ordered relation members with resolved node/way IDs and a derived `direction_id_derived`. | Route import |
| `osm_route_relation_stops.csv` | Stop-only relation members used for stop alignment and diagnostics. | Route import |
...
(
    ...
    relation(bn.pt_nodes)[type=route_master];
    relation(bw.candidate_ways)[type=route_master];
);
out meta;
```
