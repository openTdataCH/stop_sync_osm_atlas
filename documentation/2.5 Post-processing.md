...
flowchart TB
    ...
    P2 -->|remaining| P3["manual_match(ctx)<br/><i>human-curated from DB</i>"]
    P3 -->|"list of match dicts"| R1
```
...
- the ATLAS `sloid` is still unmatched (`sloid not in ctx.atlas.matched_ids`)
- the OSM node is not already used (`node_id not in ctx.osm.used_ids`)
