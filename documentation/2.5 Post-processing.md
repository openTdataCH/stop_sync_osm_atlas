...
```mermaid
flowchart LR
    subgraph Input["After 4 Stages"]
        I[Matching Results]
        U[("Used OSM<br/>node IDs")]
    end
    
    subgraph Post["Post-Processing"]
        P1["1. Unique-by-UIC<br/>(post-pass exact)<br/>+883 matches"]
        P2["2. Duplicate Propagation<br/>+66 matches"]
        P3["3. Manual Matches<br/>(from database)"]
        P1 --> P2 --> P3
    end
    
    subgraph Output["Final"]
        O["48,213 total<br/>matched pairs"]
    end
...
|-----------|---------------|-------------|
| Unique-by-UIC | +883 | Match isolated OSM nodes by UIC |
| Duplicate propagation | +66 | Spread matches across ATLAS duplicates |
...
| Metric | Count |
|--------|-------|
| Total matched pairs | 48,213 |
| Distinct matched ATLAS | 46,611 (~85%) |
| From post-processing | ~949 |
| Remaining unmatched ATLAS | 8,269 |
