...
flowchart TB
    ...
    UA --> T["atlas.get_route_evidence(sloid)<br/>Build ATLAS tokens (GTFS)"]
...
- `AtlasState._route_evidence_by_sloid` via `ctx.atlas.get_route_evidence(sloid)` — GTFS route entries loaded from `atlas_line_families.csv`, `atlas_itineraries.csv`, and `atlas_itinerary_stop_calls.csv` during `AtlasState.from_dataframe()`
...
`run_matching()` loads `RouteState` from the ATLAS line-family and OSM relation CSVs before the predicate pipeline. When that state contains a mapping for an OSM relation ID, the token builder also adds the mapped ATLAS route ID and its normalized form before intersecting it with the ATLAS tokens.
...
|--------|--------------|-----------|-------------|
| GTFS routes | `data/processed/atlas_line_families.csv`, `atlas_itineraries.csv`, `atlas_itinerary_stop_calls.csv` | `AtlasState` | Timetable-derived route entries per SLOID for stop-level matching |
| OSM routes | OSM XML relations | `OsmState.from_xml_file()` | Route memberships per OSM node (via relation ID) |
| Route equivalency state | `data/processed/atlas_line_families.csv` + `data/processed/osm_route_relations.csv` | `RouteState.load_and_match()` before the predicate pipeline | Exact/normalized ATLAS line-family crosswalk used when building OSM tokens |

## Related Documentation

- [1.2 GTFS](1.2%20GTFS%20ATLAS%20data.md) – GTFS route extraction
- [1.3 OSM data](1.3%20OSM%20data.md) – OSM route extraction
- [3.2 Route-Route Matching](3.2%20Route-Route%20Matching.md) – Route-level ATLAS↔OSM linking in importer
...
| `ctx.atlas.get_route_evidence(sloid)` | Returns ATLAS route assignments for a SLOID |
