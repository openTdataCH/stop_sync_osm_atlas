...
## Migration Policy During Active Development

While schema is still evolving quickly, we keep a single migration file baseline (`b5fa82492b15_initial_migration_for_split_databases.py`) and fold active schema updates there.

For issue #37, this includes way provenance columns on `osm_nodes` so selected OSM ways can be stored without renaming core tables.
...
`osm_nodes` now stores both native node-backed elements and selected way-backed virtual elements. Provenance is tracked via `is_way`, `source_way_id`, and optional `way_node_ids`.
