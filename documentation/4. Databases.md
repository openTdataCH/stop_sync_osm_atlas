...
While schema is still evolving quickly, we keep `b5fa82492b15_initial_migration_for_split_databases.py` as the split-database baseline and add incremental Alembic revisions for changes after that baseline.
...
`osm_nodes` currently stores node-level stop attributes and duplicate-group metadata only. Historical way-provenance columns (`is_way`, `source_way_id`, `way_node_ids`) are not part of the current schema.
