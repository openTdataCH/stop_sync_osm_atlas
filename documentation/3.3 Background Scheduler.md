...
| Prepare ATLAS | Download and filter the official stop data, or report that the verified cache was reused. |
| Prepare timetable data | Download/extract GTFS when necessary, reduce trips into patterns, resolve GTFS stops to ATLAS and construct ATLAS itineraries. |
| Prepare OpenStreetMap | Download the current extract, parse stops and route relations, and prepare route evidence. |
...
The runner streams the engine's JSON `stage_started` and `stage_progress` records while preserving them in the scheduler log. It maps those engine-owned details to the stable public phases above. Cache-decision events mark ATLAS or timetable stages as `Reused`; modes that intentionally omit work mark it as `Skipped`. Import code reports staging and the real atomic table switch directly, so publication is no longer represented by a metadata-only step after the switch.
