...
The current implementation uses one strict rule plus three fallbacks:
...
| **2. Parent-Station Broadcast** | If the GTFS `stop_id` has no platform token (`normalized_local_ref` is empty), treat it as a parent station and map it to **all** ATLAS rows sharing that UIC number. |
| **3. Unique-Number Fallback** | If strict matching fails and the ATLAS `number` appears exactly once, assign that single `sloid`. |
| **4. SLOID-Token Fallback** | Otherwise, among ATLAS rows with the same `number`, match the GTFS `normalized_local_ref` against the last token of the `sloid`. |
...
- **Key Functions**:
    ...
    - `match_gtfs_to_atlas()`: Maps GTFS `stop_id` to ATLAS `sloid` using strict matching plus unique-number and `sloid`-token fallbacks.
