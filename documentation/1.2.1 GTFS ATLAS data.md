...
The current implementation uses a strict rule plus two fallbacks:
...
| **2. Unique-Number Fallback** | If strict matching fails and the ATLAS `number` appears exactly once, assign that single `sloid`. |
| **3. SLOID-Token Fallback** | Otherwise, among ATLAS rows with the same `number`, match the GTFS `normalized_local_ref` against the last token of the `sloid`. |
