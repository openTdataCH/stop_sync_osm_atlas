...
- **atlas_routes_unified.csv** → Builds `atlas_route_dir_to_sloids`
- **osm_nodes_with_routes.csv** → Builds `osm_route_dir_to_nodes` (includes node IDs and selected virtual way IDs such as `way_<way_id>`)
...
For issue #37, selected OSM ways are converted into virtual stop elements before import and persisted in `osm_nodes` with provenance columns (`is_way`, `source_way_id`, `way_node_ids`).
