...
flowchart TB
    ...
    subgraph Import["Import Pipeline (matching_and_import_db/database/importer.py)"]
        ...
        I["4. Import Nodes, Matches, Unmatched Stops, Routes"]
        P["5. Detect Problems During Import"]
        M --> C --> R --> I --> P
    end
...
2. **Import OSM groups** into `osm_stop_groups`.
3. **Import matched stops** into `stops_matched`, creating `atlas_stops` rows on demand and evaluating matched-stop problems.
4. **Import unmatched ATLAS stops** into `stops_matched`, creating `atlas_stops` rows on demand and evaluating unmatched-stop problems.
5. **Import unmatched OSM stops** into `stops_matched` and evaluate unmatched-stop problems.
6. **Import routes** into `route_atlas_stops`, `route_osm_stops`, and `routes_matched`.
...
| Scenario | Behavior |
|----------|----------|
The separate `user_input_db` is reserved for persistence features, but those re-application semantics are not active in the current importer implementation.
