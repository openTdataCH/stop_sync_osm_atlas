...
The `import_to_database()` function in [`matching_and_import_db/database/importer.py`](file:///Users/guillemmassague/Desktop/stop_sync_osm_atlas/matching_and_import_db/database/importer.py) orchestrates the import:
...
We truncate tables based on the execution phase. Because the import database is physically separate from user data, we use `TRUNCATE CASCADE` for instant, safe deletion without orphaning user data.
...
1. **Insert Domain Nodes** – Post core node metadata into `atlas_stops` and `osm_nodes`. (Phase 1)
2. **Insert Domain Routes** – Post route sequences into `route_atlas_stops` and `route_osm_stops`. (Phase 1)
3. **Execute Route Matches** – Link identical routes across domains into `routes_matched`. (Phase 2)
4. **Execute Stop Matches** – Map and resolve heuristic matches into `stops_matched`. (Phase 3)
