...
Detection uses a **predicate pipeline** mirroring the matching pipeline architecture. Before the database write phase, `ProblemContext.build()` precomputes shared indexes (KDTrees, UIC counts, duplicate maps, and route evidence lookups) from the `MatchingOutput`. Each eligible record is then evaluated by the five problem predicates.
...
The importer skips problem evaluation for unmatched ATLAS records at the `(0, 0)` coordinate sentinel. It also skips unmatched OSM records at `(0, 0)` when their raw tags do not contain a latitude.
...
| `atlas_count_by_uic` | `dict[str, int]` | ATLAS occurrences per UIC across matched records and unmatched entities |
...
| `handled_duplicate_sloids` | `set[str]` | Individual ATLAS SLOIDs whose match record has `match_type='duplicate_propagation'` |
| `atlas_route_evidence_by_sloid` | `dict[str, dict[str, list]]` | Per-SLOID GTFS route evidence used for route-contradiction checks |
...
| Priority | Condition | Rationale |
|----------|-----------|-----------|
| **P1** | A UIC is present and `ctx.atlas_count_by_uic` has 0 entries for it | Completely missing counterpart |
...
> **Note**: A field is compared only when both sides are present. Name, local-ref, and operator comparisons are case-insensitive; UIC comparisons are exact. Each check can be individually toggled via `ENABLE_*_CHECK` constants in `context.py`. The predicate emits one `attributes` result at the highest-severity mismatch found, not one row per mismatched field.

---

## 4.1.4. Contradicts Route Matching Problems

Flag matched pairs whose own route evidence contradicts the chosen ATLAS↔OSM stop pairing. This predicate reads route evidence from `ProblemContext.atlas_route_evidence_by_sloid`, `ProblemContext.osm_node_routes`, and `ProblemContext.osm_name_dirs`, then reuses the same route-alignment classifier as the matching pipeline. Missing or inconclusive evidence does not produce a problem.
