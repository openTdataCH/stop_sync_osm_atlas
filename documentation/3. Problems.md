...
This defines how problems are detected and prioritized. Core logic lives in [matching_process/problem_detection.py](https://github.com/openTdataCH/stop_sync_osm_atlas/blob/main/matching_process/problem_detection.py). Problems are stored in the table `problems` with `priority` (1 = highest). Each problem has at most one `solution` and an `is_persistent` flag.
...
Operator normalization: see [OPERATOR_NORMALIZATION.md](https://github.com/openTdataCH/stop_sync_osm_atlas/blob/main/documentation/1.2%20OSM%20operator%20normalization.md) (OSM operators normalized before comparisons).
...
Implementation (in [import_data_db.py](https://github.com/openTdataCH/stop_sync_osm_atlas/blob/main/import_data_db.py))
...
- API groups duplicates in `/api/problems` for UI review (see [backend/blueprints/problems.py](https://github.com/openTdataCH/stop_sync_osm_atlas/blob/main/backend/blueprints/problems.py))
