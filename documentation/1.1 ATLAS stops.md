...
> [!NOTE]
> We do not download ATLAS and GTFS data on every run. Before preprocessing, the scheduler checks the source permalinks for updated file validators. The current preprocessing step is shared: it reuses the existing ATLAS + GTFS artifacts only when both sources are unchanged; otherwise it reruns the combined preprocessing job. The reason is due to SLOID<->GTFS stop_id matching. More info in [6.3 Background Scheduler](6.3%20Background%20Scheduler.md).
>
...
- during GTFS `stop_id` <-> `sloid` import via `gtfs_stop_identity_resolution`
