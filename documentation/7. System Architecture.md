...
flowchart LR
    subgraph DS [Data Stores]
        ...
        State[(file runtime dir or redis)]
    end
...
- `redis`: Optional shared backend for multi-worker rate limiting and Redis-backed pipeline state.
...
| `redis` or file runtime dir | shared pipeline-status and lock backend |
...
- pipeline execution uses `PIPELINE_STATE_BACKEND` with either file-backed state or Redis

The shared pipeline-state backend stores:
...
- [7.4 Frontend Asset Management](7.4%20Frontend%20Asset%20Management.md): Vendored frontend libraries, Font Awesome optimization, and sync pipeline
- [7.5 Atlas-Cached Import Optimization](7.5%20Atlas-Cached%20Import%20Optimization.md): Table-reuse behavior for unchanged ATLAS/GTFS inputs
