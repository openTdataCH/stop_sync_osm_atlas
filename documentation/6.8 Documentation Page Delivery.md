...
3. Resolve the active page from its canonical slug. Unknown or legacy filename-style paths return 404.
...
repo_scanner.py performs a lazy, one-time scan when documentation auto-linking first needs it and builds a filename-to-path index for code files. The scanner ignores directories such as `documentation/`, `data/`, `vendor/`, virtual environments, build output, and generated caches. It does scan application code under `static/` and `templates/`.
