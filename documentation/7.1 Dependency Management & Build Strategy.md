...
- `requirements-test.txt`: Testing frameworks (`pytest`, `pytest-flask`, `pytest-cov`).
...
Automated safeguards for doc drift:
- `tests/test_dependency_docs_sync.py` validates that dependency lists in this document remain synchronized with all `requirements-*.txt` files.
