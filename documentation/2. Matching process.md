...
| `osm_group_siblings` | `dict[str, tuple[str, list[OsmNode]]]` | Platform ↔ stop_position pairs (representative → (group_type, siblings)) |
...
4. Select the platform as the representative for each pair (the stop_position becomes the sibling)
...
- The DB stores `osm_group_uic` or `osm_group_name` as the `match_type` to distinguish how the group was formed
