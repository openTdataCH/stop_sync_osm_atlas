...
For `osm_trio` groups, `ctx.commit()` intentionally skips OSM sibling propagation so the trio middle node remains unmatched unless another predicate explicitly matches it. During the database import phase, if both of its side nodes are successfully matched, the importer promotes the middle node directly to `effectively_matched` for fast querying.
...
After matching, the importer separately flags unmatched ATLAS rows that have no OSM node at all within 50 m. That does **not** create additional matches; it feeds [problem detection](4.%20Problems.md).
