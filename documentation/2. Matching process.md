...
3. Run the 8 predicates in order
...
| `OsmState` | `osm_data.xml` | Parse OSM nodes plus selected way-derived stop elements and route relations, build attribute indexes, build spatial index on demand |
...
Issue #37 adds selective way ingestion before index finalization. A way is converted into a virtual stop element (`way_<way_id>`) only if:

1. it has `aerialway=station` and `public_transport=station`, or
2. it has `uic_ref` and no existing OSM node has the same `uic_ref`.

Kept ways are represented as point-like entries (Overpass `center` or centroid fallback) and enter the same indexes as nodes, including route membership and direction strings.
...
Only node-backed OSM entries participate in this pre-grouping step. Way-derived virtual stops (`is_way=true`, issue #37) remain matchable as standalone entities but are excluded from pair/trio construction.

| Group type | Representative | Sibling(s) | Core rule |
|------------|----------------|------------|-----------|
| `osm_trio` | One non-middle side node | Side partner + identified middle | UIC-scoped trio: exactly 3 OSM nodes + exactly 1 stop_position + exactly 2 ATLAS rows for that UIC |
...
2. For perfect-count anchors (`left_count == right_count == atlas_effective_count`), grouping first tries reciprocal conflict-free pairing within `15 m` and bypasses ratio checks.
3. `atlas_effective_count` is computed per UIC by counting only ATLAS rows whose nearest same-UIC OSM node is within `30 m`.
4. The strict/relaxed fallback path still uses the original (unfiltered) `atlas_count`.
5. If the perfect-count branch does not apply or cannot produce a full 1:1 pairing, it tries a `1.5` ratio threshold and accepts those pairs only when grouped + ungrouped OSM entities exactly match the ATLAS count for that anchor.
6. If the strict complete-count check fails, it retries with `2.0` and accepts reciprocal pairs it finds as an incomplete group set.
7. In ratio-based paths, the ratio test compares the nearest candidate (which must be within `12 m`) against the true second-nearest candidate, even if that second candidate is beyond `12 m`. If no second candidate exists at all, the ratio test does not reject the pair.
8. The same perfect-count-first then strict-vs-incomplete policy applies to UIC, name, and tram pairing paths.
...
```mermaid
flowchart LR
    T["2.1 Trio distance"] --> E["2.2 Exact"] --> N["2.3 Name"] --> D1["2.4a Group proximity"] --> D2["2.4b Local ref"] --> D3["2.4c Nearest (3a)"] --> D4["2.4d Nearest (3b)"] --> D5["2.4e Nearest (3a retry)"] --> R["2.5 Route"] --> P["2.6 Post-pass"]
```
...
| `2.4a` | [`GroupProximityPredicate`](2.3%20Distance%20matching.md) | `distance_matching_1_uic_ref`, `distance_matching_1_uic_name`, `distance_matching_1_name` | Conflict-free maximum-cardinality assignment within grouped candidates (distance <= 50m) |
| `2.4b` | [`LocalRefDistancePredicate`](2.3%20Distance%20matching.md) | `distance_matching_2` | Exact `designation == local_ref` within 50 m |
| `2.4c` | [`NearestDistancePredicate`](2.3%20Distance%20matching.md) | `distance_matching_3a` | Single-candidate proximity within 50 m (pass 1) |
| `2.4d` | [`NearestDistancePredicate`](2.3%20Distance%20matching.md) | `distance_matching_3b` | Ratio-test proximity within 50 m |
| `2.4e` | [`NearestDistancePredicate`](2.3%20Distance%20matching.md) | `distance_matching_3a_second_pass` | Single-candidate proximity retry after ratio matches |
| `2.5` | [`RouteMatchPredicate`](2.4%20Route%20Matching.md) | `route_unified_gtfs`, `route_unified_hrdf` | Still limited to 50 m, then tries GTFS route tokens, HRDF direction-UIC evidence, and direction-name fallback |
| `2.6` | [`PostpassUniqueUicPredicate`](2.5%20Post-processing.md) | `exact_postpass` | Last pass for `1 ATLAS + 1 OSM` left for a UIC when the OSM node has no `local_ref` |
...
For `osm_trio` groups, `ctx.commit()` intentionally skips OSM sibling propagation so the trio middle node remains unmatched unless another predicate explicitly matches it.
...
| `MatchingOutput` | `PipelineResult` fields plus `duplicate_sloid_map`, `osm_pairs`, `osm_trios`, `all_osm_nodes` | Pipeline output plus pre-pipeline grouping state needed by the importer |
...
The batched KDTree path is used by the local-ref, nearest-distance, and route predicates. `GroupProximityPredicate` does not use the KDTree; it builds a full pairwise distance matrix with NumPy broadcasting and solves a conflict-free max-cardinality assignment.
