...
Later stages only see what earlier stages could not confidently match.
...
- **Osm Platform/Stop-position** Some ATLAS platforms might be represented by two OSM nodes: one with the tag "public_transport= platfrom" and antoher with the tag "public_transport= stop_position"
...
*Note: This exclusion is applied globally across the Exact, Name, and Distance matching stages to ensure matches map to specific stopping points rather than general station nodes.*
