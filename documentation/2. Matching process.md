...
**OSM groups (platform ↔ stop_position pairs) are transparent at the `MatchRecord` level.** During matching, only one node from each group participates in predicate evaluation. When it is matched, its partner is automatically locked. Both group members get their own `stops_matched` row during database import — the partner row is created with the same ATLAS match but its own OSM coordinates and independently computed `distance_m`.
...
| `osm_groups` | `list[tuple[str, str, str]]` | Platform ↔ stop_position pairs as `(node_id_1, node_id_2, group_type)` |
...
`OsmState.build_groups()` pre-pairs `public_transport=platform` nodes with nearby `public_transport=stop_position` nodes before any predicate runs. Two grouping paths run sequentially:
...
**Effect on matching:**
- During matching, only one node from each group participates in predicate evaluation (the other is hidden from query methods)
- When the participating node is matched via `mark_used()`, its partner is automatically locked too
- Both group members get their own `stops_matched` row during database import — the partner's row has the same ATLAS match but its own OSM coordinates and computed `distance_m`
- Unmatched group members both appear as `osm_unmatched`
- The `osm_stop_groups` table stores the pairing with `group_type`: `osm_group_uic` or `osm_group_name`
