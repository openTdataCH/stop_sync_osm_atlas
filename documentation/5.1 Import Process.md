...
`precompute_problem_artifacts()` builds the shared `ProblemContext`, stores matched results on each `MatchRecord`, and creates lookup maps for unmatched results needed later by the payload builder.
...
- `atlas_cached` rewrites only the dynamic tables and validates that the required static ATLAS/GTFS tables already exist and are non-empty
...
The importer submits inserts in chunks controlled by `DB_IMPORT_BATCH_SIZE`, but all selected `TRUNCATE` and `INSERT` statements run in one database transaction. The refresh commits once at the end and rolls back as a unit if any write fails.
...
| `DB_IMPORT_BATCH_SIZE` | Records per bulk-insert chunk within the refresh transaction | `10000` |
...
```bash
export DATABASE_URI="postgresql+psycopg://user:pass@localhost/stopdb"
python -m matching_and_import_db.database.importer
```

The direct importer exports stats after writing, but it does not call the scheduler's `_record_data_updated_timestamp()` helper. Consequently, it does not update `last_pipeline_data_import_ended_at` in `data/data_meta.json`.

## Persistence Behavior

The importer validates that the payload contains at least one `stops_matched` row before truncating anything. Once a refresh proceeds, it does not preserve manual edits in the rewritten table scope; the import database is expected to be reproducible from the current files and matching output.
