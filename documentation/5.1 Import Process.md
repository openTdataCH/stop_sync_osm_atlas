...
- **dynamic rows** rewritten every run: OSM tables, `stops_matched`, `problems`, normalized route tables, and route match tables
...
2. `osm_stops`
3. `osm_stop_members`
4. static ATLAS/GTFS tables in `complete` mode only:
   - `atlas_operators`
   - `atlas_stops`
   - `gtfs_stops_raw`
