...
flowchart TB
    ...
    CTX --> OSM["osm.batch_query_radius()<br/><i>incl. stations, excl. used</i>"]
    CTX --> DIRS["osm.name_dirs / osm.uic_dirs"]

    UA --> T["Build ATLAS tokens<br/>(GTFS + HRDF)"]
    T --> LOOP["For each ATLAS entry"]
    LOOP --> C["Find OSM candidates within 50m"]
...
**Internal Context:**
- `osm_nodes`: full OSM node set (candidates found via batched `OsmState.batch_query_radius()` within `max_distance`)
...
All functions are in [route_matching_unified.py](../matching_and_import_db/route_matching_unified.py).
