...
|--------|--------------|-----------|-------------|
| GTFS | `data/processed/atlas_routes_gtfs.csv` | `AtlasState.from_dataframe()` | Timetable-derived routes per SLOID |
...
|----------------|-------------|
| `RouteMatchPredicate` | Predicate class; builds tokens, uses `batch_query_radius()` candidates, matches by priority |
...
All predicate logic is in [predicates/route_matching_gtfs.py](../matching_and_import_db/predicates/route_matching_gtfs.py). Route data loading lives in [state.py](../matching_and_import_db/state.py).
