...
Matched against direction UIC strings extracted from OSM route relations (first/last member UIC refs), stored in `OsmState.uic_dirs`.

### P3: Name-Based Direction Fallback

ATLAS direction names compared against OSM route relation direction strings (first/last member names like `"Zurich HB → Bern"`), stored in `OsmState.name_dirs`.
