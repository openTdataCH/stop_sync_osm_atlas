...
flowchart TB
    ...
    UA --> T["atlas.get_routes(sloid)<br/>Build ATLAS tokens (GTFS + HRDF)"]
...
When spatial proximity fails, route information provides an alternative: if an ATLAS platform and an OSM node share strong GTFS route-token evidence or compatible direction evidence, they likely represent the same stop.
...
- `AtlasState._routes_by_sloid` via `ctx.atlas.get_routes(sloid)` — GTFS + HRDF route entries loaded from `atlas_routes_unified.csv` during `AtlasState.from_dataframe()`
...
ATLAS tokens are built as `{(line_name, direction_uic)}`, but the current matcher only checks the `direction_uic` component against direction UIC strings extracted from OSM route relations (first/last member UIC refs), stored in `OsmState.uic_dirs`. `line_name` is carried through the ATLAS data but is not currently enforced at match time.
...
| HRDF | `data/processed/atlas_routes_unified.csv` | `AtlasState.from_dataframe()` | Railway-specific route data per SLOID |
...
(Route provenance is tracked in the output via `match_type` — e.g. `route_unified_gtfs` or `route_unified_hrdf` — and accompanying evidence in `notes`.)
...
| `ctx.atlas.get_routes(sloid)` | Returns `{'gtfs': [...], 'hrdf': [...]}` route entries for a SLOID |
