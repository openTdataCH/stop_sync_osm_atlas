# Persistent Data

The persistent data system preserves user-created solutions and notes across data imports, allowing regular refreshes while maintaining institutional knowledge.

```mermaid
flowchart TD
    subgraph Import[Data Import]
        I1[Clear Operational Tables] --> I2[Write Fresh Data]
        I2 --> I3[Apply Persistence]
    end
    
    subgraph Persist[Persistent Storage]
        P1[(persistent_data)]
        P1 --> I3
    end
    
    subgraph User[User Actions]
        U1["Save Solution/Note"] --> U2{"Make Persistent?"}
        U2 -->|Yes| P1
        U2 -->|No| U3["Non-persistent only"]
    end
```

## Data Types

| Type | Behavior on Import |
|------|-------------------|
| **Operational** (stops, problems) | Cleared and repopulated |
| **Persistent** (solutions, notes) | Preserved and re-applied |

## Database Schema

| Column | Type | Description |
|--------|------|-------------|
| `sloid` | String | ATLAS identifier |
| `osm_node_id` | String | OSM identifier |
| `problem_type` | String | 'distance', 'unmatched', 'attributes', 'duplicates' |
| `solution` | String | Resolution description |
| `note_type` | String | 'atlas' or 'osm' |
| `note` | String | User annotation |
| `created_by_user_id` | Integer | Author user ID |
| `created_at` | DateTime | Creation timestamp |

**Unique constraint**: `(sloid, osm_node_id, problem_type, note_type)`

## Persistence Scenarios

| Scenario | Result |
|----------|--------|
| Stop exists, same problem | Solution re-applied |
| Stop exists, problem gone | Solution retained for future |
| Stop removed | Solution orphaned; may apply if stop returns |

## API Endpoints

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/save_solution` | POST | Save solution (non-persistent) |
| `/api/make_solution_persistent` | POST | Mark as persistent |
| `/api/save_note/{atlas\|osm}` | POST | Save note |
| `/api/make_note_persistent/{type}` | POST | Mark note persistent |
| `/api/persistent_data` | GET | List all persistent items |
| `/api/clear_all_persistent` | POST | Remove all (admin) |

## Best Practices

✅ **Use for**: Confirmed exceptions, known differences, important context

❌ **Avoid for**: Temporary issues, data-dependent decisions

### Maintenance

1. **Periodic review**: Check orphaned items that no longer match any stop
2. **User attribution**: Track who made decisions for accountability
3. **Clean-up**: Remove outdated persistent items that clutter the system

## Permissions

Access to persistence features depends on user role—see [6.1 Permissions](6.1%20Permissions%20and%20Roles.md) for details.
