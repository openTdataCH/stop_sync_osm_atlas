...
- **Same zoom + viewport remains inside the buffered request bounds** → Reuse
- **Zoom in + previous response was not capped + viewport remains inside the buffered request bounds** → Reuse
- **Zoom out, a capped zoom change, or movement beyond the buffered request bounds** → Fetch new data
- **Filters changed** → Fetch new data

Implementation details:

- `MapViewportLoader` stores the last response data, visible bounds, buffered request bounds, zoom, and request identity
- the request identity is built from the viewport policy mode plus active filter parameters and `limit`; exact `zoom`, `offset`, `include_meta`, and bounding-box parameters are intentionally excluded
- representing zoom through the mode/limit policy allows an uncapped buffered result to remain reusable while zooming in
- bounding-box coordinates are kept separately in the buffered request bounds and are not part of the identity
- capped state is derived from the cached response metadata when deciding whether a zoom-in reuse is safe
...
The `MarkerClusterManager` handles stops at identical or near-identical coordinates. When a Leaflet map projection is available, it projects coordinates at a stable reference zoom, groups nearby points using a spatial grid, and offsets each group in pixel space:
...
Entries are sorted by entity type and stable key before layout, which keeps displacement deterministic. The configured coordinate tolerance (`0.00001`) is only the fallback grouping threshold when map projection is unavailable. At lower zooms, subpixel offsets are skipped because they would not be visible.
...
| [backend/importing/importer.py](../backend/importing/importer.py) | Geometry import |

## Related Documentation

- [A2 Web app](2.%20Web%20app.md) – Application overview
- [A1 Database & Bundle Import](1.%20Database.md) – Database schema
