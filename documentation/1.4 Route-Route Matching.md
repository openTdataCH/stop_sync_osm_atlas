...
Route-route matching runs early in the pipeline as part of the `RouteState` singleton initialization, before any stop-matching predicates are executed.

```mermaid
flowchart LR
    A[Load entity-first route CSVs] --> B[Initialize RouteState]
    B --> C[Compute Route Equivalencies]
    C --> D[Run stop matching predicates]
    D --> E[import_to_database]
    E --> F[Bulk write routes_matched]
```

Concretely, route linkage is computed in `matching_and_import_db/route_state.py` and saved into the `RouteState` module. Later, `matching_and_import_db/database/route_loader.py` retrieves this mapping to prepare the `routes_matched` bulk-insert payload for `importer.py`.

## Inputs

Route-route matching depends on the entity-first route metadata:

- `atlas_routes.csv` and `atlas_route_directions.csv`
- `osm_routes.csv`
