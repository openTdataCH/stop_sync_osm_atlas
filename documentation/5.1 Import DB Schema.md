...
| `group_kind` | String(50) | NULL for single; pair/trio subtype (`osm_pair_*` or `osm_trio`) |
| `representative_node_id` | String(100) | **FK** -> `osm_nodes.osm_node_id` (CASCADE). Deterministic representative node |
...
| Column | Type | Description |
|--------|------|-------------|
| `osm_stop_id` | Integer | **FK** -> `osm_stops.id` (CASCADE) |
| `node_id` | String(100) | **FK** -> `osm_nodes.osm_node_id` (CASCADE). Also globally UNIQUE across stop units |
...
Provides the ordered mapping of ATLAS stops natively to their GTFS routes.
...
| Table | Index | Purpose |
|-------|-------|---------|
| `osm_stops` | `stop_kind` | Fast scope filtering between single/pair/trio units |
| `osm_stops` | `group_kind` | Fast subgroup filtering (`osm_pair_*`, `osm_trio`) |
| `osm_stops` | `representative_node_id` | Fast representative lookup for popup/render defaults |
| `osm_stop_members` | `node_id` (UNIQUE) | Fast node -> stop-unit lookup and exclusivity guarantee |
...
- **[5. Databases](5.%20Databases.md)** – Database architecture and reproducibility
- **[5.2 Import Process](5.2%20Import%20Process.md)** – Process for transforming pipeline output and CSV files into database records
