...
The system is designed to re-import base data periodically (e.g., daily) to stay synchronized with ATLAS and OSM updates. The Import DB is fully rebuilt on each run, while the User Input DB (auth data, user notes) is kept separate and never wiped.
...
**Import DB** is wiped and rebuilt on every pipeline run. **User Input DB** persists across runs — it stores auth data and user notes. Solution persistence (feeding user corrections back into the pipeline) is modelled but not yet active. See [5. Databases](5.%20Databases.md) for details.
...
| **[8. CI and Tests](8.%20GITHUB_ACTIONS_AND_TESTS.md)** | GitHub Actions, test setup, and local checks |
