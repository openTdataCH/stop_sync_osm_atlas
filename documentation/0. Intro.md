...
To accurately link Swiss official data with OpenStreetMap, the system performs four distinct types of matching. The primary objective is to link **ATLAS sloid** to **OSM stops**, but this requires several supporting layers:
...
3.  **GTFS stop_id ↔ ATLAS sloid**: ATLAS GTFS data uses `stop_id` as a primary key rather than `sloid`, necessitating a mapping layer to connect timetable data to official platform IDs.
