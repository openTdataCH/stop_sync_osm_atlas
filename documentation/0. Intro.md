...
The two parts have separate responsibilities and can run independently. The engine reads source and OSM inputs, matches stops and routes, detects problems, and writes results. The web app imports those results and provides the database, API, and review interface.

```mermaid
flowchart LR
    E["Matching engine<br/>Input integrations · matching · results"]
    B["Versioned result bundle"]
    A["Web application<br/>Orchestration · import · PostGIS<br/>API · review interface"]
    E --> B --> A
```

The scheduler belongs to the web app. It starts the engine CLI as a separate process; the app does not import engine code at runtime. The versioned result bundle is the data boundary between them.
...
```mermaid
flowchart TB
    subgraph Engine["Matching engine — CLI process"]
        direction LR
        B["Source and OSM inputs<br/>Dataset profile"] --> E["Match stops and routes<br/>Detect problems"]
    end
    Engine --> R["Versioned result bundle"]
    subgraph App["Web application"]
        direction LR
        I["Validate, stage and publish<br/>database snapshot"] --> D[("PostGIS")]
        D --> W["API and review website"]
    end
    R --> App
```
