...
flowchart LR
    ...
    O --> D
    I --> IDB
    W -->|"User solutions &amp; notes"| UIB
    UIB -.->|"Persistent data<br/>reapplied on import"| M
```

**Import DB** is wiped and rebuilt on every pipeline run. **User Input DB** persists across runs — it stores user solutions, notes, and auth data created via the web app, and feeds persistent solutions back into the matching pipeline. See [4. Databases](4.%20Databases.md) for details.
