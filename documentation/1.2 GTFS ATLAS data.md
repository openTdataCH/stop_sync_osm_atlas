...
The result is integrated into `data/processed/atlas_routes_gtfs.csv`.
...
| `route_name_long` | Long route name (often empty in current CH feed) | `` |
...
`route_id_normalized` is produced by replacing route segments matching `-j<digits>` with `-jXX` (for example `-j24`, `-j25`, `-j123`). Other route-id segments are preserved as-is.

> [!NOTE]
> `routes.txt` currently exposes additional useful fields (`agency_id`, `route_desc`, `route_type`) that are available in source but not yet exported into `atlas_routes_gtfs.csv`.
