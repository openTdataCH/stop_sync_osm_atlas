...
`route_id_normalized` is currently produced by replacing year tokens that match `-jNN` (for example `-j24`, `-j25`) with `-jXX`. Other route-id segments are preserved as-is.
