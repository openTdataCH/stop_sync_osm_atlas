...
The primary goal of this process is to generate entity-first route files (**`data/processed/atlas_routes.csv`**, **`atlas_route_directions.csv`**, and **`atlas_route_stops.csv`**). These files map ATLAS stops to their associated transit routes and directions using GTFS (General Transit Feed Specification) timetable data.
...
- **Total GTFS Stops Processed**: {{stat:gtfs_mapping.total_gtfs_stop_ids}}
- **Matched GTFS Stops**: {{stat:gtfs_mapping.matched_gtfs_stop_ids}} ({{stat:gtfs_mapping.gtfs_coverage_percent}}%)
- **Unmatched GTFS Stops**: {{stat:gtfs_mapping.unmatched_gtfs_stop_ids}}
- **Strict Matching Assignments**: {{stat:gtfs_mapping.strict_assignments}}
- **Unique-Number Fallback Assignments**: {{stat:gtfs_mapping.unique_number_fallback_assignments}}
...
The result is integrated into three normalized CSV files in `data/processed/`:

### 1. `atlas_routes.csv`
Defines the route entities.
