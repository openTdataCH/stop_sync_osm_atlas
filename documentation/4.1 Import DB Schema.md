...
erDiagram
    ...
    osm_nodes ||--o{ osm_pairs : "node_id_1 FK"
    osm_nodes ||--o{ osm_pairs : "node_id_2 FK"
    osm_nodes ||--o{ osm_trios : "middle_node_id FK"
    osm_nodes ||--o{ osm_trios : "side_node_id_1 FK"
    osm_nodes ||--o{ osm_trios : "side_node_id_2 FK"
    ...
    osm_trios {
        int id PK
        string middle_node_id FK
        string side_node_id_1 FK
        string side_node_id_2 FK
    }
...
| `match_type` | String(50) | Examples: `exact`, `exact_postpass`, `name`, `distance_matching_trio`, `distance_matching_*`, `route_unified_*`, `duplicate_propagation`, `osm_group_propagation`, `no_nearby_counterpart` |
...
Duplicate membership for ATLAS is sourced from `duplicate_group_sloids` (canonical), not from `stops_matched`.
...
| `is_way` | Boolean | `true` if this row comes from a selected OSM way represented as a virtual stop element |
| `source_way_id` | String(100) | Original OSM way ID when `is_way=true` |
| `way_node_ids` | JSONB | Optional list of member node IDs from the source way |
| `osm_node_type` | String(50) | Derived type: `platform`, `stop_position`, `railway_station`, `ferry_terminal`, `aerialway` (`aerialway=station` is classified as `aerialway`) |
...
Duplicate membership for OSM is sourced from `duplicate_group_node_ids` (canonical), not from `stops_matched`.

For issue #37, selected way elements are persisted in `osm_nodes` using virtual IDs (`way_<way_id>`) and `is_way=true` provenance.

---

## Table: `osm_pairs`

Platform / stop_position pairs identified by the pre-matching grouping pass (see [2. Matching process](2.%20Matching%20process.md), OSM Node Grouping section). Each row links two OSM nodes that form a pair. Both members are equal - when one is matched to an ATLAS stop, both get their own `stops_matched` row via `osm_group_propagation`. Coordinates are retrieved from the `osm_nodes` table when needed.

Pair construction is node-only: rows with `osm_nodes.is_way=true` are intentionally excluded from `osm_pairs` generation.
...
| `group_type` | String(50) | How the pair was formed: `osm_pair_uic`, `osm_pair_name`, `osm_pair_tram`, `osm_pair_uic_equal_15m`, `osm_pair_name_equal_15m`, or `osm_pair_tram_equal_15m` |

---

## Table: `osm_trios`

UIC-scoped three-node OSM groups identified before pair grouping. Each row stores one explicit middle node and two side nodes. Trio rows are used by trio-first distance matching and by API rendering helpers that draw middle-to-side connectors.

Trio construction is also node-only: way-derived virtual stops (`is_way=true`) are not eligible as trio side or middle members.

| Column | Type | Description |
|--------|------|-------------|
| `id` | Integer | Primary key |
| `middle_node_id` | String(100) | **FK** -> `osm_nodes.osm_node_id` (CASCADE). Explicit trio middle node |
| `side_node_id_1` | String(100) | **FK** -> `osm_nodes.osm_node_id` (CASCADE). First trio side node |
| `side_node_id_2` | String(100) | **FK** -> `osm_nodes.osm_node_id` (CASCADE). Second trio side node |
...
| Table | Index | Purpose |
|-------|-------|---------|
| `osm_pairs` | `node_id_1` | Fast lookup of pair partner for a matched stop (used by `/api/data` query) |
| `osm_pairs` | `node_id_2` | Bidirectional lookup (either member can be queried) |
| `osm_trios` | `middle_node_id` | Lookup trios by middle node |
| `osm_trios` | `side_node_id_1` | Lookup trios by side node |
| `osm_trios` | `side_node_id_2` | Lookup trios by side node |
