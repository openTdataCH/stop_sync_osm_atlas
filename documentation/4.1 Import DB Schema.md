...
erDiagram
    ...
    osm_nodes ||--o{ osm_stop_groups : "representative_node_id FK"
    osm_nodes ||--o{ osm_stop_groups : "sibling_node_id FK"
...
| `match_type` | String(50) | `exact`, `exact_postpass`, `name`, `distance_matching_*`, `route_*`, `duplicate_propagation`, `no_nearby_counterpart` |
...
Platform / stop_position sibling pairs identified by the pre-matching grouping pass (see [2. Matching process](2.%20Matching%20process.md), OSM Node Grouping section). Each row links a **representative** OSM node to one of its **sibling** nodes. Coordinates are denormalized so the `/api/data` endpoint can return group-line geometry via a single `IN` query without joining back to `osm_nodes`.

| Column | Type | Description |
|--------|------|-------------|
| `id` | Integer | Primary key |
| `representative_node_id` | String(100) | **FK** → `osm_nodes.osm_node_id` (CASCADE). The node that participates in matching |
| `sibling_node_id` | String(100) | **FK** → `osm_nodes.osm_node_id` (CASCADE). The grouped-away node |
| `group_type` | String(50) | How the group was formed: `osm_group_uic` (shared UIC) or `osm_group_name` (shared name) |
| `sibling_lat` | Float | Latitude of the sibling node (denormalized for line rendering) |
| `sibling_lon` | Float | Longitude of the sibling node (denormalized for line rendering) |
...
| Table | Index | Purpose |
|-------|-------|---------|
| `osm_stop_groups` | `representative_node_id` | Fast lookup of siblings for a matched stop (used by `/api/data` IN query) |
| `osm_stop_groups` | `sibling_node_id` | Reverse lookup (e.g. "is this node a sibling?") |
...
Tables that survive data re-imports (such as `persistent_data` and `user_notes`) are stored in a separate database schema.

See [4.2 User Input DB](4.2%20User%20Input%20DB.md) for details on the user input database and how persistence works.
