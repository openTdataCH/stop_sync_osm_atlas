...
The route comparison logic itself is documented in [3. Routes](3.%20Routes.md). This chapter stops at the files written under `data/raw/` and `data/processed/`.
...
- **Source-side route artifacts** consumed by `matching_and_import_db/database/route_loader.py` or retained as detailed OSM PTv2 source data
...
|--------|----------------|
| `matching_and_import_db/downloader/get_atlas_data.py` | ATLAS download and filtering, GTFS orchestration, ATLAS itinerary reconstruction, and ATLAS route CSV generation |
| `matching_and_import_db/downloader/get_atlas_gtfs.py` | GTFS extraction, Swiss stop-time streaming, GTFS `stop_id` resolution, identity-cache generation, and integrated GTFS/ATLAS data construction |
...
- **[3. Routes](3.%20Routes.md)**: How the importer turns those source artifacts into shared route families, itineraries, and route matches.
