...
flowchart LR
   ...
   B --> C[precompute_route_artifacts]
   C --> D[build_route_write_payload]
   D --> E[import_to_database]
   E --> F[Bulk write route_osm_stops]
   E --> G[Bulk write route_atlas_stops]
   E --> H[Bulk write routes_matched]
```

Concretely, route linkage payloads are assembled in `matching_and_import_db/database/route_loader.py` and consumed by `matching_and_import_db/database/importer.py` during the route bulk-insert phase.

## Inputs

Route-route matching depends on two preloaded mappings built by `load_all_route_data()`:

- `atlas_route_dir_to_sloids` from `data/processed/atlas_routes_gtfs.csv`
