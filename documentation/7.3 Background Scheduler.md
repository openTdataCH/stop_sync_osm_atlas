...
```mermaid
%%{init: {'gantt': {'useWidth': 1200, 'leftPadding': 140, 'rightPadding': 80, 'fontSize': 12}}}%%
gantt
...
Before executing any pipeline phase, the job runner acquires a shared,
file-backed JSON lock protected with `fcntl.flock()`. Docker Compose mounts the
same `./data` directory into the app and scheduler, so both containers observe
`data/runtime/pipeline_lock.json`. This prevents two triggers — for example a
scheduled run and a manual `docker exec` — from running simultaneously.
...
2. **When the scheduler fires the `EVENT_SCHEDULER_STARTED` event** — to ensure the shared status file has the correct value even if the scheduler loop takes a moment to begin.
...
| `STATE_BACKEND` | Runtime-state backend | `file` (fixed by Docker Compose) |
...
2. The shared status JSON is set to `{ status: "failed", phase: "failed", last_error: "..." }`.
...
| `pipeline_state_store.py` | File-backed runtime state used by Docker Compose, plus alternate store implementations used outside Compose |
