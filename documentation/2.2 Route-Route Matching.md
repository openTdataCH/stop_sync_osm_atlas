...
- [3.3 Stop-stop matching using routes](3.3%20Stop-stop%20matching%20using%20routes.md) is stop-level matching (ATLAS stop <-> OSM stop)
...
3. synthetic key based on `route`, `ref`, `operator`, and `network`
...
4. if neither side has resolved physical stop identity for a call, matching falls back to shared `uic_ref`
...
- It does not run fuzzy text similarity between route names.
- It does not run geometry scoring.
- It does not consume the stop-matching predicate scores directly.

Its job is deterministic family matching plus ordered stop-sequence pairing using the already prepared stop identities.
