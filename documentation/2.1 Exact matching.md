...
```mermaid
flowchart TB
    A["ATLAS platforms<br/>(group by number = UIC)"] --> G
    B["OSM nodes<br/>(uic_ref index)"] --> G
    U(("Used OSM<br/>node IDs")) --> G

    G["For each UIC group"] --> F["Filter OSM candidates:<br/>- not in Used OSM node IDs<br/>- not station-level"]
    F --> C{"How many candidates?"}

    C -->|0 OSM| X["No match<br/>→ next stage"]
    C -->|1 OSM, N ATLAS| M1["Match ALL ATLAS rows<br/>to this single OSM node<br/>(many-to-one)"]
    C -->|N OSM, 1 ATLAS| M2["Match this ATLAS row<br/>to ALL OSM nodes<br/>(one-to-many)"]
    C -->|N OSM, M ATLAS| D["Try to pair by<br/>designation == local_ref<br/>(case-insensitive)"]

    D -->|paired| M3["Paired matches<br/>(1:1 within the group)"]
    D -->|unpaired remain| X
...
- Unmatched ATLAS rows (forwarded to Name matching)
...
|----------|-------------|
| `exact_matching()` | Main entry point for exact matching stage |
| `match_groups()` | Applies scenario-specific logic to UIC groups |
| `disambiguate_by_local_ref()` | Resolves multiple-to-multiple cases |

All functions are in [exact_matching.py](../matching_process/exact_matching.py).
