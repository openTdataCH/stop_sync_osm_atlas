...
ATLAS entries without a matching `designation`/`local_ref` remain unmatched in this stage. If several OSM candidates share the same matching `local_ref`, the predicate takes the first still-unused candidate in source order. It checks `ctx.osm.is_used()` to avoid double-booking an OSM node within the many-to-many case.
