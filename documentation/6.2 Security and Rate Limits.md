...
Pipeline scheduling status and run locking no longer reuse `RATELIMIT_STORAGE_URI`. Those concerns now use a separate pipeline-state backend configured with `PIPELINE_STATE_BACKEND`, `PIPELINE_STATE_REDIS_URL`, and `PIPELINE_STATE_DIR`.
