...
The Problems page (`/problems`) surfaces data quality issues detected during the matching pipeline ([Section 3](3.%20Problems.md)) and presents them for review. Users can browse, filter, and navigate through problems while viewing the affected stops on an interactive map.
...
| **Solution status** | Sub-options in dropdown | Show all, solved only, or unsolved only |
...
Filters are combined server-side. Changing any filter resets pagination and fetches fresh data from `/api/problems`.
...
|----------|--------|---------|
| `/api/problems` | GET | Paginated problem list with filters |
...
| `solution_status` | `all` | Filter: `all`, `solved`, `unsolved` |
...
| `static/js/pages/problems-solutions.js` | `ProblemsSolutions` | Solution saving (localStorage drafts), persistence to server |
...
- [3. Problems](3.%20Problems.md) — Problem detection and prioritization pipeline
- [3.1 Stop Problems](3.1%20Stop%20Problems.md) — Stop-level problem predicates (distance, attributes, unmatched, duplicates)
- [5. Web app](5.%20Web%20app.md) — Web application overview
- [5.1 Map Data Loading](5.1%20Map%20Data%20Loading%20and%20Rendering.md) — Map rendering pipeline (shared components)
